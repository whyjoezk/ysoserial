...
import dogeser.payloads.DynamicDependencies;
import dogeser.payloads.ObjectPayload;
import dogeser.test.CustomPayloadArgs;
import dogeser.test.payloads.TestHarnessTest.ExecMockPayload;
import dogeser.test.payloads.TestHarnessTest.NoopMockPayload;
import dogeser.payloads.util.ClassFiles;
...
public class PayloadsTest {
    ...
    public static void testPayload ( final Class<? extends ObjectPayload<?>> payloadClass, final Class<?>[] addlClassesForClassLoader )
        ...
        dogeser.payloads.annotation.PayloadTest t = payloadClass.getAnnotation(dogeser.payloads.annotation.PayloadTest.class);
        ...
        if ( testHarness instanceof dogeser.test.CustomDeserializer) {
        ...
        if ( testHarness instanceof dogeser.test.WrappedTest) {
            callable = ( (WrappedTest) testHarness ).createCallable(callable);
        }

        if (testHarness instanceof dogeser.test.CustomTest) {
    ...
    private static Callable<byte[]> makeSerializeCallable ( final Class<? extends ObjectPayload<?>> payloadClass, final String command ) {
        return new Callable<byte[]>() {
            ...
            public byte[] call () throws Exception {
                ...
                byte[] serialized =  dogeser.Serializer.serialize(f);
    ...
    static Object deserializeWithDependencies ( byte[] serialized, final String[] dependencies, final Class<?>[] classDependencies, final Class<?> customDeserializer )
        ...
        URLClassLoader isolatedClassLoader = new URLClassLoader(urls, null) {

            {
                ...
                byte[] deserializerClassBytes = ClassFiles.classAsBytes(dogeser.Deserializer.class);
                defineClass(dogeser.Deserializer.class.getName(), deserializerClassBytes, 0, deserializerClassBytes.length);
